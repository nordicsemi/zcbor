		...
		zcbor_state_t *state, struct Pet *result)
{
	...
	bool res = (((zcbor_list_start_decode(state) && ((((zcbor_list_start_decode(state) && ((zcbor_multi_decode(1, ZCBOR_PET_DEFAULT_MAX_QTY, &(*result).names_count, ZCBOR_CUSTOM_CAST_FP(zcbor_tstr_decode), state, (*&(*result).names), sizeof(struct zcbor_string))
	...
	|| (((*result).species_choice == Pet_species_other_c) && ((1)))) || (zcbor_error(state, ZCBOR_ERR_WRONG_VALUE), false)))))
	&& zcbor_list_end_decode(state)) || (zcbor_list_map_end_force_decode(state), false)))));
